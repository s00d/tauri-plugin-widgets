...
# Usage:
#   bash scripts/sh/build-example-artifacts.sh            # all (skip unavailable)
#   bash scripts/sh/build-example-artifacts.sh macos ios android
#   pnpm build:example [platforms…]
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
# Optional local secrets (team, App Group, JAVA_HOME, …)
if [[ -f "$ROOT/scripts/apple.env" ]]; then
  set -a
  # shellcheck disable=SC1091
  source "$ROOT/scripts/apple.env"
  set +a
fi
...
if want android; then
  ...
  # AGP 8.x does not support JDK 26 — pin 17 (or 21) for Gradle.
  if command -v /usr/libexec/java_home >/dev/null 2>&1; then
    if JAVA17="$(/usr/libexec/java_home -v 17 2>/dev/null)"; then
      export JAVA_HOME="$JAVA17"
      export PATH="$JAVA_HOME/bin:$PATH"
      log "Android: JAVA_HOME=$JAVA_HOME"
    elif JAVA21="$(/usr/libexec/java_home -v 21 2>/dev/null)"; then
...
EOS
    ...
    log "Linux: SKIP (need docker image widgets-linux — run: bash scripts/sh/linux-up.sh once / docker build -t widgets-linux tests/linux)"
  fi
fi
